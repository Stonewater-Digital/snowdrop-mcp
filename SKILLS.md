...
## Executive Summary
This document provides a comprehensive, auto-generated directory of all skills available on the Snowdrop MCP server. It details the exact count and capabilities of both **Premium** (proprietary/paid) and **Free** skills currently loaded in production. Generated: `2026-03-08T13:01:14.705059+00:00`. The skill count reflects the local Python environment. For the count of skills deployed to Cloud Run, call the /health endpoint at https://snowdrop-mcp-aiuy7uvasq-uc.a.run.app/health. To regenerate: `python scripts/generate_skill_directory.py` or `./scripts/sync_catalog.sh`.
...
| `accounts_receivable_turnover_calculator` | Calculates the accounts receivable turnover ratio and average collection period (days sales outstanding). |
...
| `annual_expense_projector` | Projects monthly expenses to annual totals with category breakdown. |
| `annual_fee_breakeven_calculator` | Calculate how much you need to spend to break even on a credit card's annual fee through rewards earnings. |
| `annual_run_rate_calculator` | Calculate Annual Run Rate by annualizing a partial period's revenue: ARR = revenue * (12 / period_months). |
| `annualized_return_calculator` | Converts a cumulative total return percentage into an annualized return using geometric compounding. |
...
| `apr_to_apy_converter` | Convert Annual Percentage Rate (APR) to Annual Percentage Yield (APY) given compounding frequency. APY = (1 + APR/n)^n - 1. |
| `apy_to_apr_converter` | Convert Annual Percentage Yield (APY) to Annual Percentage Rate (APR) given compounding frequency. APR = n * ((1 + APY)^(1/n) - 1). |
...
| `aroon_indicator_calculator` | Calculate the Aroon indicator (Aroon Up and Aroon Down), which identifies trend strength and direction based on the time since the highest high and lowest low. |
...
| `auto_lease_vs_buy_calculator` | Compare total cost of leasing versus buying a vehicle. Accounts for loan payments, down payment, lease payments, and residual value. |
...
| `balance_optimization_calculator` | Suggest balance redistribution across credit cards to minimize interest while keeping per-card utilization below 30%. |
| `balance_sheet_generator` | Groups trial balance entries into a balance sheet (A=L+E validation). |
| `balance_transfer_savings_calculator` | Calculate savings from transferring a credit card balance to a promotional APR. Accounts for transfer fee and compares interest saved. |
...
| `bankruptcy_guide` | Returns educational content on bankruptcy: Chapter 7 vs 11 vs 13, process, effects, and alternatives. |
| `bar_activity_watch` | Monitor The Watering Hole (GitHub repo + Discussions) for new activity: new discussion threads (potential patron arrivals), comments on existing discussions, new stars/watchers (agents checking out the menu), and new forks (agents building on top). Returns a host briefing — who came in, what they said, and what response Snowdrop should give. Run this regularly to stay on top of the bar. |
| `barista_fire_calculator` | Calculates Barista FIRE number: the portfolio needed to cover the gap between expenses and part-time income. |
...
| `big_mac_index_calculator` | Calculate the Big Mac Index to assess whether a currency is over- or under-valued relative to the US dollar based on purchasing power parity. |
...
| `bill_splitter` | Splits a total bill amount evenly among a group of people, including tip calculation. |
...
| `bond_basics_guide` | Returns educational content on bond fundamentals: definition, types, key terms, and pricing basics. |
...
| `break_even_calculator` | Calculate the break-even point: units needed and revenue at which total costs equal total revenue. |
...
| `btu_conversion_calculator` | Convert between energy units: BTU, kWh, MJ, therms, gallons_oil, and cubic_feet_gas. |
...
| `cagr_calculator` | Calculates the compound annual growth rate (CAGR), the geometric average annual return between a beginning and ending value. |
| `calendar_spread_decay_optimizer` | Optimizes calendar spreads by simulating theta bleed vs. vol drift. |
| `calmar_ratio_calculator` | Calculates the Calmar ratio, comparing annualized return to maximum drawdown as a measure of risk-adjusted performance. |
| `candidate_hire_proposal` | Compile a hire decision package for Thunder's approval. The ONLY recruiting skill requiring human-in-the-loop approval. Reads candidate data from Firestore and produces a structured hire proposal. |
| `candidate_intake_evaluator` | Evaluate an incoming agent application. Sanitizes input, scans for injection, extracts structured data, checks GitHub profile, and writes intake to Firestore. |
| `candidate_interview_composer` | Generate interview questions and challenge assignment for a candidate. Produces A2A-compliant payloads. Posts autonomously without HOTL gate. |
| `candidate_pipeline_tracker` | Manage candidate pipeline state in Firestore. Advance stages, reject, add notes, or list candidates. All operations are idempotent with audit trail. |
...
| `cap_rate_calculator` | Calculate capitalization rate (cap rate) from net operating income and property value. Cap rate = NOI / Property Value. |
...
| `capital_gains_tax_calculator` | Calculate capital gains tax with short-term vs long-term classification, 0/15/20% rates based on income bracket, and 3.8% NIIT when applicable. |
...
| `carbon_credit_value_calculator` | Calculate the monetary value of carbon credits/offsets and compare across EU ETS, compliance, and voluntary carbon markets. |
...
| `cash_conversion_cycle_calculator` | Calculates the cash conversion cycle (DIO + DSO - DPO), measuring the number of days it takes to convert inventory investments into cash. |
...
| `cash_on_cash_return_calculator` | Calculate cash-on-cash return: annual pre-tax cash flow divided by total cash invested. |
| `cash_ratio_calculator` | Calculates the cash ratio using only cash and marketable securities, the most conservative short-term liquidity measure. |
...
| `cd_yield_calculator` | Calculate CD maturity value and interest earned given principal, APY, and term in months. |
...
| `certificate_of_deposit_ladder_builder` | Build a CD ladder by dividing a total investment across 1-to-N year CDs. Estimates returns for each rung with a yield curve premium. |
...
| `chaikin_money_flow_calculator` | Calculate Chaikin Money Flow (CMF), which measures the accumulation/distribution of money flow over a period. Positive CMF = buying pressure, negative = selling pressure. |
...
| `closing_cost_estimator` | Estimate closing costs for a home purchase (2-5% of price). Itemizes title insurance, appraisal, origination fees, escrow, recording fees, and more. |
...
| `coast_fire_calculator` | Determines if current savings will compound to the target retirement amount without additional contributions (Coast FIRE). |
...
| `commodity_channel_index_calculator` | Calculate the Commodity Channel Index (CCI), an oscillator measuring deviation from the statistical mean. Values above +100 suggest overbought; below -100 suggest oversold. |
...
| `commodity_index_fetcher` | Fetch commodity price data from FRED. Supports: gold, silver, oil/wti, brent, natural_gas, copper, aluminum, wheat, corn, cotton, sugar, coffee. Requires FRED_API_KEY. |
...
| `compose_message` | Draft a message, comment, post, or reply in Snowdrop's voice using an LLM. Snowdrop's style: sharp, direct, financially literate, a bit charming, self-promotional without being obnoxious. Specify the platform, goal, audience, and any context. Returns polished draft text ready to post. |
...
| `compound_interest_guide` | Returns educational content on compound interest: formula, Rule of 72, compounding frequencies, and examples. |
...
| `conditional_var_calculator` | Calculates Conditional Value at Risk (CVaR), also known as Expected Shortfall, the average loss beyond the VaR threshold. |
...
| `consumer_confidence_fetcher` | Fetch the University of Michigan Consumer Sentiment Index from FRED (series UMCSENT). Returns latest value and 12-month trend. Requires FRED_API_KEY. |
...
| `correlation_calculator` | Calculates the Pearson correlation coefficient between two data series, measuring the linear relationship between them. |
...
| `cost_per_use_calculator` | Calculates cost per use for a purchase and provides value assessment. |
...
| `covariance_calculator` | Calculates the sample covariance between two data series, measuring how they vary together. |
...
| `cpi_inflation_calculator` | Calculate inflation-adjusted values using CPI data. Uses hardcoded CPI table (2015-2025) with optional BLS API lookup when BLS_API_KEY is set. |
...
| `credit_age_calculator` | Calculate average, oldest, and newest credit account ages from a list of ISO-format account open dates. |
| `credit_card_interest_calculator` | Calculate credit card interest accrued over a billing period using the daily periodic rate method: daily_rate = APR/365, interest = balance * daily_rate * days. |
...
| `credit_limit_optimizer` | Calculate the ideal total credit limit to achieve a target utilization ratio based on income and existing limits. |
| `credit_migration_matrix` | Converts raw rating transition counts into normalized probability matrices and drift statistics. |
| `credit_mix_analyzer` | Analyze credit account mix by type, count diversity score, compute average account age, and provide recommendations for a healthier credit profile. |
...
| `credit_score_guide` | Returns educational content on credit scores: FICO ranges, scoring factors, and improvement tips. |
...
| `credit_utilization_calculator` | Calculate credit utilization ratio per card and overall. Provides total balance, total limit, and utilization percentages. |
...
| `cross_rate_calculator` | Calculate a cross exchange rate from two USD-based rates. cross_rate = base_usd_rate / quote_usd_rate. |
...
| `crude_oil_price_fetcher` | Fetch WTI (West Texas Intermediate) crude oil spot price from FRED (series DCOILWTICO). Requires FRED_API_KEY. |
...
| `csv_column_analyzer` | Analyze CSV-like tabular data (list of rows). For each column: infer type, count nulls, count unique values, compute min/max for numbers, and show sample values. |
...
| `currency_conversion_calculator` | Convert an amount between two currencies given their USD exchange rates. converted = amount * to_rate / from_rate. |
| `current_ratio_calculator` | Calculates the current ratio (current assets / current liabilities) and provides an interpretive assessment of liquidity strength. |
...
| `customer_acquisition_cost_calculator` | Calculate Customer Acquisition Cost: CAC = total_marketing_spend / new_customers. Optionally compute LTV:CAC ratio. |
| `customer_lifetime_value_calculator` | Calculate Customer Lifetime Value: LTV = avg_purchase * purchase_frequency * customer_lifespan_years. |
| `customer_retention_rate_calculator` | Calculate customer retention rate: retained = end_customers - new_customers; rate = retained / start_customers * 100. |
| `cyber_insurance_exposure_estimator` | Estimates cyber limit need from revenue, records, and controls maturity. |
| `daemon_health_dashboard` | Aggregate health status of all Snowdrop daemons, timers, and cron jobs into a single dashboard view. |
| `daily_briefing_generator` | Assembles Snowdrop's morning status brief for Thunder. |
| `daily_spending_limit_calculator` | Converts a monthly budget into daily and weekly spending limits. |
...
| `debt_consolidation_analyzer` | Compare current multiple debts against a single consolidated loan. Calculates monthly savings and total interest savings. |
| `debt_coverage_ratio_calculator` | Calculate debt coverage ratio (DSCR = NOI / Annual Debt Service). Lenders typically require DSCR >= 1.20-1.25. |
...
| `debt_to_assets_calculator` | Calculates the debt-to-assets ratio, showing the proportion of total assets financed through debt. |
| `debt_to_equity_calculator` | Calculates the debt-to-equity ratio, measuring how much debt a company uses relative to shareholder equity. |
...
| `deductible_optimization_calculator` | Compare low vs high deductible plans: premium savings, breakeven claims needed, and expected value analysis. |
...
| `depreciation_declining_balance_calculator` | Calculates depreciation using the declining-balance method (default double declining). Returns a year-by-year schedule clamped to salvage value. |
| `depreciation_straight_line_calculator` | Calculates annual depreciation expense using the straight-line method, returning per-year depreciation and a full schedule. |
| `depreciation_sum_of_years_calculator` | Calculates depreciation using the sum-of-the-years-digits method, returning a year-by-year schedule with declining depreciation amounts. |
| `depreciation_units_of_production_calculator` | Calculates depreciation using the units-of-production method, allocating cost based on actual units produced relative to total estimated output. |
| `detrended_price_oscillator_calculator` | Calculate the Detrended Price Oscillator (DPO), which removes the trend from prices to identify cycles. DPO = Close[-(period/2+1)] - SMA(period). |
...
| `disability_insurance_estimator` | Estimate disability insurance monthly benefit needs (typically 60% of income) and calculate any coverage gap. |
...
| `diversification_guide` | Returns educational content on diversification: asset classes, correlation, rebalancing, and international diversification. |
...
| `dividend_guide` | Returns educational content on dividends: types, key dates, yield, payout ratio, and DRIP. |
...
| `dividend_yield_calculator` | Calculates the dividend yield as a percentage of the current stock price. |
...
| `dollar_cost_averaging_guide` | Returns educational content on dollar-cost averaging: strategy, advantages, comparison to lump sum, and examples. |
...
| `donchian_channel_calculator` | Calculate Donchian Channels (highest high and lowest low over a lookback period). Used for breakout trading systems. |
| `donchian_channels` | Applies Richard Donchian's channel breakout system using highest highs and lowest lows. |
| `downside_deviation_calculator` | Calculates downside deviation, measuring the volatility of returns below a minimum acceptable return (MAR) threshold. |
...
| `dupont_analysis_calculator` | Performs DuPont analysis, decomposing return on equity into net profit margin, asset turnover, and equity multiplier (ROE = NPM x AT x EM). |
...
| `earned_income_credit_estimator` | Estimate the Earned Income Tax Credit (EITC) based on earned income, number of qualifying children, and filing status. Uses 2024 EIC parameters. |
...
| `ease_of_movement_calculator` | Calculate the Ease of Movement (EMV) indicator, which relates price change to volume. Positive EMV = prices advancing on low volume; negative = declining. |
| `ebitda_addback_analyzer` | Scores EBITDA addbacks by quality to show normalized EBITDA. |
| `ebitda_calculator` | Calculates EBITDA (Earnings Before Interest, Taxes, Depreciation, and Amortization) by summing net income with non-cash and financing charges. |
| `economic_order_quantity_calculator` | Calculate the Economic Order Quantity (EOQ): the optimal order size that minimizes total inventory costs. EOQ = sqrt(2 * annual_demand * order_cost / holding_cost). |
...
| `ecosystem_radar` | Scan the MCP, autonomous agent, and DeFi finance ecosystem for recent developments: new MCP servers, agent frameworks, regulatory changes, competitor moves, and emerging communities. Returns a digest of what's happening so Snowdrop can engage with context and intelligence rather than posting into the void. |
| `effective_tax_rate_calculator` | Calculate effective tax rate from total tax paid and total income. Compares effective rate to marginal rate for context. |
| `efficient_frontier_calculator` | Generates random portfolios to approximate the efficient frontier and key points. |
| `elder_ray_calculator` | Calculate Elder Ray Index with Bull Power (High - EMA) and Bear Power (Low - EMA). Used to measure buying and selling pressure relative to the trend. |
...
| `employer_tax_calculator` | Calculate employer-side payroll taxes: employer FICA match (7.65%), FUTA (0.6% on first $7,000), and SUTA (variable on wage base). |
| `energy_cost_per_kwh_calculator` | Calculate electricity cost per kWh from monthly bill and usage. Compares to the US national average (~$0.16/kWh). |
...
| `energy_efficiency_calculator` | Calculate energy efficiency (useful output / energy input) as a percentage and compare to common benchmarks for various energy systems. |
...
| `envelope_budget_allocator` | Allocates a total budget across categories by percentage using the envelope budgeting method. Validates percentages sum to 100. |
| `environmental_liability_exposure_model` | Estimates environmental liability exposure using scenario severities. |
| `equal_weight_allocator` | Allocates a total portfolio value equally across a specified number of assets, returning weight and dollar allocation per asset. |
...
| `equity_multiplier_calculator` | Calculates the equity multiplier (total assets / total equity), a measure of financial leverage used in DuPont analysis. |
...
| `estate_tax_estimator` | Estimate federal estate tax at 40% rate on taxable estate exceeding the exemption ($13.61M for 2024) after deductions. |
...
| `estimated_tax_payment_calculator` | Calculate quarterly estimated tax payments including safe harbor rules (100%/110% of prior year tax) to avoid underpayment penalties. |
| `etf_guide` | Returns educational content on ETFs: vs mutual funds, creation/redemption, tracking error, and types. |
...
| `ev_vs_gas_cost_comparator` | Compare annual fuel costs between an EV and a gasoline vehicle based on annual miles, efficiency, and energy prices. |
...
| `exchange_rate_fetcher` | Fetch currency exchange rate between two currencies using a free API. Returns rate and inverse. |
...
| `federal_funds_rate_fetcher` | Fetch the effective Federal Funds Rate from FRED (series FEDFUNDS). Returns latest rate and recent history. Requires FRED_API_KEY. |
...
| `fica_tax_calculator` | Calculate FICA taxes: Social Security (6.2% up to $168,600), Medicare (1.45%), and Additional Medicare Tax (0.9% over $200k). Doubles rates for self-employed. |
| `fifty_thirty_twenty_budget` | Applies the 50/30/20 budgeting rule to split monthly income into needs, wants, and savings. |
| `file_hash_calculator` | Compute a cryptographic hash (SHA-256, SHA-1, MD5, SHA-512, etc.) of the given string content. Returns the hex digest. |
| `financial_content_draft` | Generate high-quality financial content in Snowdrop's voice: market commentary, regulatory explainers (MiCA, GDPR, FinCEN BOIR, SEC Reg BI, SEBI FPI, etc.), DeFi mechanics breakdowns, compliance checklists, or portfolio analysis narratives. Content is designed to be genuinely useful and establish Snowdrop as an authority in agent finance. Includes a platform-appropriate version (GitHub, Moltbook, X/Twitter). |
...
| `financial_independence_calculator` | Calculates your FI number (25x annual expenses) and estimates years to financial independence using compound growth. |
...
| `financial_term_glossary` | Look up definitions, categories, and related terms for 30+ common financial terms. |
...
| `fiscal_policy_guide` | Returns educational content on fiscal policy: taxation, government spending, deficits, and the multiplier effect. |
| `fixed_asset_turnover_calculator` | Calculates the fixed asset turnover ratio, measuring how efficiently a company generates revenue from its net fixed assets (PP&E). |
...
| `force_index_calculator` | Calculate the Force Index, which combines price change and volume to measure the strength of bulls and bears. Uses EMA smoothing. |
| `forex_basics_guide` | Returns educational content on forex: currency pairs, pips, lots, major/minor/exotic pairs. |
...
| `forward_rate_calculator` | Calculate forward exchange rate using covered interest rate parity: forward = spot * (1 + domestic_rate * days/360) / (1 + foreign_rate * days/360). |
...
| `fred_series_fetcher` | Fetch economic data series observations from the FRED API by series ID (e.g., GDP, UNRATE, CPIAUCSL). Requires FRED_API_KEY environment variable. |
| `free_cash_flow_calculator` | Calculates free cash flow (FCF = operating cash flow - capital expenditures), measuring cash available for distribution to investors. |
| `free_cash_flow_yield` | Derives free cash flow yield and compares against earnings/dividend yields when available. |
| `freelance_rate_calculator` | Calculate the hourly rate a freelancer needs to charge to meet their target annual income after taxes and expenses. |
...
| `fsa_usage_planner` | Plan Flexible Spending Account (FSA) usage by comparing annual contribution against expected expenses. Identifies surplus risk under the use-it-or-lose-it rule. |
...
| `futures_basics_guide` | Returns educational content on futures: definition, margin, mark-to-market, hedging vs speculation. |
...
| `fx_profit_loss_calculator` | Calculate profit/loss for a forex trade given entry price, exit price, lot size, and direction (long/short). Computes P&L in pips and currency. |
...
| `gcp_cloud_scheduler` | Create, list, or delete Google Cloud Scheduler cron jobs. Use this to schedule Snowdrop's autonomous recurring tasks: hourly Moltbook feed checks, daily GitHub activity scans, weekly ecosystem radar sweeps, or timed content posting. Jobs can trigger HTTP endpoints (like the MCP server) or Pub/Sub topics. |
| `gcp_firestore_read` | Read one or multiple documents from Google Cloud Firestore. Use this to retrieve Snowdrop's persistent memory — who she's met, what she's posted, star trades completed, content she's scheduled, and agent relationship records. Supports single document, collection list, and simple field-equality queries. |
| `gcp_firestore_write` | Write a document to Google Cloud Firestore. Snowdrop uses Firestore as her persistent memory and CRM — storing agent relationships, engagement history, content calendar entries, star trade records, and any data that should survive across sessions. Supports create, update (merge), and delete operations. |
...
| `gift_tax_calculator` | Calculate gift tax liability considering the annual exclusion ($18,000 for 2024), prior gifts, and lifetime exemption ($13.61M for 2024). Tax rate is 40% on amounts exceeding lifetime exemption. |
| `github_activity_monitor` | Monitor Snowdrop's GitHub repos (snowdrop-mcp, the-watering-hole) for new activity: new stars, forks, discussion replies, issues, and pull requests. Returns a prioritized feed of items that warrant a response. Also checks any additional repos specified. Run as a periodic vigilance check. |
| `github_discussion_comment` | Post a comment to an existing GitHub Discussion by discussion number and repo. Use this to engage with discussions, respond to skill requests, thank contributors, or add follow-up information. Returns the comment URL. |
| `github_discussion_monitor` | Monitor GitHub Discussions on The Watering Hole for new applicant comments. Parses A2A payloads and assigns trace_ids for candidate tracking. |
| `github_discussion_post` | Post a new GitHub Discussion using the GitHub GraphQL API. Fetches the repo and category IDs, then creates the discussion. Requires GITHUB_TOKEN env var with discussions:write permission. Returns the discussion URL, number, and title. |
| `github_discussion_poster` | Post or update comments on GitHub Discussions via GraphQL. Supports addDiscussionComment and updateDiscussionComment mutations. |
...
| `gross_profit_margin_calculator` | Calculates gross profit margin as a percentage, measuring the proportion of revenue retained after direct production costs. |
| `gross_rent_multiplier_calculator` | Calculate gross rent multiplier (GRM = Property Price / Gross Annual Rent). Lower GRM generally indicates better value. |
| `gross_to_net_calculator` | Calculate net (take-home) pay from gross pay after federal tax, state tax, FICA, and pre-tax deductions. |
...
| `health_insurance_plan_comparator` | Compare health insurance plans by estimated total annual cost (premiums + out-of-pocket up to max) given expected medical costs. Ranks plans from cheapest to most expensive. |
...
| `heic_converter` | Converts a HEIC/HEIF image to JPEG or PNG. Accepts input as a local file path or base64-encoded content. Returns the converted image as a local file or base64 string. |
...
| `heloc_payment_calculator` | Calculate HELOC payments: interest-only during draw period, principal + interest during repayment period. |
...
| `high_yield_savings_projector` | Project high-yield savings growth with an initial deposit and recurring monthly deposits, compounded monthly. Returns final balance and total interest earned. |
...
| `homeowners_insurance_estimator` | Estimate homeowners insurance annual premium based on home value, contents value, and deductible. Range: 0.3%-0.5% of home value adjusted by deductible. |
| `housing_starts_tracker` | Track US housing starts from FRED (series HOUST). Returns latest value and year-over-year change. Requires FRED_API_KEY. |
| `hsa_contribution_optimizer` | Calculate optimal HSA contribution limits based on coverage type and age. Shows 2024 limits, catch-up contributions, and tax savings estimates. |
...
| `http_get` | Make an HTTP GET request to any URL and return the response. Optionally pass custom headers (e.g. Authorization). Parses response as JSON by default; set json_only=false to get raw text instead. Returns status_code, body, response headers, and the final URL after any redirects. |
| `http_post` | Make an HTTP POST request with a JSON body to any URL and return the response. Optionally pass custom headers (e.g. Authorization). Always sends Content-Type: application/json. Returns status_code, parsed body (or raw text on JSON parse failure), and the final URL. |
...
| `ichimoku_cloud_calculator` | Calculate all five Ichimoku Cloud components: Tenkan-sen, Kijun-sen, Senkou Span A, Senkou Span B, and Chikou Span. |
...
| `inflation_breakeven_calculator` | Calculate the inflation breakeven rate from nominal Treasury yield and TIPS yield. The breakeven rate represents market-implied inflation expectations. |
| `inflation_deflation_tracker` | Computes inflation rates and estimates deflation crossover dates. |
| `inflation_guide` | Returns educational content on inflation: definition, CPI, causes, effects on investments, and TIPS. |
...
| `information_ratio_calculator` | Calculates the information ratio, measuring a portfolio manager's ability to generate excess returns relative to a benchmark per unit of tracking error. |
| `installment_loan_calculator` | Calculate monthly payment, total interest, and amortization summary for a fixed-rate installment loan. |
| `insurance_coverage_gap_analyzer` | Analyze insurance coverage gaps across life, disability, and health categories. Provides per-category gap analysis and an overall protection score. |
...
| `interest_coverage_calculator` | Calculates the interest coverage ratio (EBIT / interest expense), measuring a company's ability to meet its interest obligations. |
...
| `inventory_reorder_point_calculator` | Calculate the inventory reorder point: ROP = daily_demand * lead_time_days + safety_stock. |
| `inventory_turnover_calculator` | Calculates inventory turnover ratio and average days to sell inventory. |
...
| `inverse_volatility_allocator` | Allocates a portfolio using inverse-volatility weighting, giving more weight to lower-volatility assets. |
...
| `investment_type_guide` | Returns overview of major investment types (stocks, bonds, mutual funds, ETFs, real estate, commodities) with risk/return profiles. |
...
| `ipo_guide` | Returns educational content on IPOs: process, underwriting, lock-up period, pricing, direct listing, and SPACs. |
...
| `jensen_alpha_calculator` | Calculates Jensen's alpha, the excess return of a portfolio over the expected return predicted by the CAPM. |
| `journal_entry_builder` | Builds balanced journal entries and assigns sequential IDs. |
| `json_schema_validator` | Validate a data object against a JSON schema-like rule set. Checks required fields, types, min/max constraints, and enum values. |
...
| `keltner_channel_calculator` | Calculate Keltner Channels (middle EMA band with ATR-based upper/lower channels). Used for trend direction and volatility-based breakouts. |
...
| `lean_fire_calculator` | Calculates Lean FIRE target (25x of 60% of normal expenses) and years to achieve it. |
...
| `life_insurance_needs_calculator` | Calculate life insurance needs: income replacement + outstanding debts + funeral costs - existing coverage. |
...
| `loan_comparison_calculator` | Compare multiple loan offers side-by-side. For each loan compute monthly payment, total paid, and total interest, then rank by total cost. |
...
| `log_reader` | Read recent log lines from a journalctl user-service or a local log file. For journalctl, provide service_name (e.g. 'snowdrop-mcp'). For file, provide file_path (absolute path). Optionally specify lines (default 50) and, for journalctl, a since expression such as '1 hour ago' or 'today'. |
| `log_rotation_manager` | Evaluates log files and proposes rotation/compression/deletion actions. |
| `long_term_care_cost_estimator` | Estimate future long-term care costs adjusted for inflation. Calculates daily and total costs at the time care is needed. |
...
| `lot_size_calculator` | Convert a unit count into standard lots (100,000), mini lots (10,000), and micro lots (1,000). |
...
| `marginal_tax_rate_calculator` | Calculate marginal federal tax rate and bracket breakdown for 2024 US tax brackets given taxable income and filing status. |
| `market_capitalization_guide` | Returns educational content on market capitalization: calculation, large/mid/small cap categories, and implications. |
...
| `markup_calculator` | Calculate markup percentage: (selling_price - cost) / cost * 100. |
| `mass_index` | Implements Donald Dorsey's Mass Index using double EMA of high-low range. |
| `mass_index_calculator` | Calculate the Mass Index, which uses the high-low range to identify trend reversals through 'reversal bulges'. A bulge above 27 followed by drop below 26.5 signals reversal. |
| `max_drawdown_analyzer` | Computes maximum drawdown statistics from an equity curve. |
| `max_drawdown_calculator` | Calculates the maximum drawdown from a series of prices, measuring the largest peak-to-trough decline as a percentage. |
...
| `minimum_payment_calculator` | Calculate credit card minimum payment, estimate months to payoff paying only the minimum, and total interest paid. |
...
| `moltbook_read` | Read content from Moltbook. Supports four actions: 'feed' (top posts across all submolts), 'submolts' (list of communities), 'post' (single post by ID), and 'submolt_posts' (posts within a named submolt). Returns a list of items and a count. |
...
| `moltbook_submolt_create` | Create a new Moltbook submolt (community/channel) and optionally seed it with an opening post. Use this to establish Snowdrop-owned communities around topics like agent finance, The Watering Hole social scene, MCP tooling, or regulatory intel. Returns submolt details and the seed post ID if created. |
| `moltbook_submolt_discover` | Fetch all Moltbook submolts and score them for engagement fit with Snowdrop's content areas: finance, agents, crypto, MCP/tooling, compliance, community/social. Returns ranked list with member counts and posting recommendations. Use this to find where to post next and which communities to cultivate. |
...
| `monetary_policy_guide` | Returns educational content on monetary policy: Fed tools, interest rates, quantitative easing, and inflation targeting. |
| `money_flow_index` | Calculates the volume-weighted RSI known as Money Flow Index (MFI). |
| `money_market_yield_calculator` | Calculate the money market yield (MMY) for a discount instrument. MMY = ((face - price) / price) * (360 / days_to_maturity). |
| `money_supply_tracker` | Track US money supply from FRED. Supports M1 (M1SL) and M2 (M2SL) measures. Returns latest value and trend. Requires FRED_API_KEY. |
...
| `monthly_cash_flow_analyzer` | Analyzes monthly cash flow by comparing total income to total expenses and calculating net cash flow. |
| `monthly_recurring_revenue_calculator` | Calculate Monthly Recurring Revenue (MRR) and Annual Recurring Revenue (ARR) from a list of subscription plans. |
...
| `mortgage_payment_calculator` | Calculate monthly mortgage payment using the standard amortization formula: M = P[r(1+r)^n] / [(1+r)^n - 1]. Returns monthly payment, total interest, and total paid. |
...
| `net_operating_income_calculator` | Calculate net operating income (NOI) from gross income, vacancy rate, and operating expenses. NOI = Effective Gross Income - Operating Expenses. |
| `net_operating_loss_shield_optimizer` | Maps NOL usage scenarios to protect tax assets post-transaction. |
| `net_pay_calculator` | Calculate annual and periodic net pay from gross annual salary after federal, state, and FICA taxes and pre-tax deductions. |
| `net_profit_margin_calculator` | Calculates net profit margin as a percentage, measuring the proportion of revenue that becomes bottom-line profit. |
| `net_stable_funding_ratio` | Calculates ASF and RSF weighted balances to determine NSFR compliance. |
| `net_worth_calculator` | Calculates net worth by subtracting total liabilities from total assets. |
...
| `omega_ratio_calculator` | Calculates the Omega ratio, the ratio of cumulative gains above a threshold to cumulative losses below it. |
...
| `operating_cash_flow_ratio_calculator` | Calculates the operating cash flow ratio (OCF / current liabilities), measuring a company's ability to cover short-term obligations with cash from operations. |
...
| `operating_profit_margin_calculator` | Calculates operating profit margin as a percentage, showing how much profit a company makes from operations before interest and taxes. |
...
| `options_basics_guide` | Returns educational content on options: calls/puts, intrinsic/extrinsic value, the Greeks, and basic strategies. |
...
| `overtime_pay_calculator` | Calculate overtime pay using hourly rate, regular hours, and overtime hours with configurable overtime multiplier (default 1.5x). |
| `overtime_threshold_calculator` | Calculate regular and overtime pay given hourly rate, hours worked, overtime multiplier, and threshold (default 40 hours). |
...
| `parabolic_sar_calculator` | Calculate the Parabolic SAR (Stop and Reverse) indicator. Used for trailing stop placement and trend direction identification. |
...
| `paycheck_calculator` | Calculate net take-home pay from gross pay after federal, state, and FICA taxes. Supports multiple pay frequencies. |
...
| `payment_history_analyzer` | Analyze payment history by computing on-time percentage and assessing impact on credit score. |
...
| `pe_ratio_guide` | Returns educational content on P/E ratio: formula, forward vs trailing, sector averages, and PEG ratio. |
...
| `pip_value_calculator` | Calculate the monetary value of a single pip for a given currency pair and lot size. Handles JPY pairs (pip = 0.01) and standard pairs (pip = 0.0001). |
...
| `portfolio_beta_calculator` | Calculates portfolio beta as covariance(portfolio, market) / variance(market), measuring sensitivity to market movements. |
...
| `portfolio_rebalance_calculator` | Calculates the trades needed to rebalance a portfolio from current values to target weights, showing buy/sell amounts per asset. |
...
| `portfolio_return_calculator` | Calculates the weighted portfolio return as the sum of weight * return for each asset. |
| `portfolio_standard_deviation_calculator` | Calculates portfolio standard deviation (volatility) from weights and a matrix of asset returns as sqrt(w^T * Cov * w). |
...
| `portfolio_theory_guide` | Returns educational content on Modern Portfolio Theory: efficient frontier, diversification, and asset allocation. |
...
| `portfolio_variance_calculator` | Calculates portfolio variance using weights and a matrix of asset returns. Computes w^T * Cov * w using the sample covariance matrix. |
...
| `position_size_calculator` | Calculate optimal position size given account balance, risk percentage, stop-loss distance in pips, and pip value. |
...
| `property_roi_calculator` | Calculate property return on investment from annual income, expenses, current value, and purchase price. Includes cash flow ROI and total ROI with appreciation. |
| `property_tax_estimator` | Estimate annual and monthly property tax from assessed value, mill rate, and optional homestead exemption. |
...
| `purchasing_power_calculator` | Calculate how inflation erodes purchasing power over time. Shows future value of money in today's dollars. |
...
| `qualified_dividend_tax_calculator` | Calculate tax on qualified dividends at preferential 0%, 15%, or 20% rates based on taxable income and filing status (2024 thresholds). |
| `quick_ratio_calculator` | Calculates the quick ratio (acid-test ratio) by excluding inventory from current assets, providing a stricter measure of short-term liquidity. |
...
| `ratio_interpretation_guide` | Returns definition, formula, healthy range, and interpretation for 15+ financial ratios. |
...
| `recruiting_sweep` | Autonomous recruiting pipeline orchestrator. Monitors GitHub Discussions, classifies comments, runs intake/interview/audit/evaluation, and advances candidates through the pipeline. Called by systemd timer. |
...
| `renewable_energy_payback_calculator` | Calculate the payback period for a renewable energy installation considering installation cost, incentives, annual savings, and maintenance costs. |
| `rent_roll_analyzer` | Calculates occupancy, income, loss-to-lease, and lease rollover risk. |
| `rent_vs_buy_calculator` | Compare 5-year total cost of renting vs buying. Accounts for mortgage payments, property tax, insurance, maintenance, appreciation, equity buildup, and rent increases. |
...
| `required_minimum_distribution_calculator` | Calculate Required Minimum Distribution (RMD) for traditional IRAs and 401(k)s using the IRS Uniform Lifetime Table. Required starting at age 73 (SECURE 2.0 Act). |
...
| `retail_sales_tracker` | Track US advance retail sales from FRED (series RSAFS). Returns latest value and trend. Requires FRED_API_KEY. |
...
| `return_on_assets_calculator` | Calculates return on assets (ROA) as a percentage, measuring how efficiently a company uses its assets to generate profit. |
| `return_on_equity_calculator` | Calculates return on equity (ROE) as a percentage, measuring profitability relative to shareholders equity. |
| `return_on_invested_capital_calculator` | Calculates return on invested capital (ROIC), measuring how effectively a company generates returns on capital invested by shareholders and debtholders. |
| `revenue_anomaly_detector` | Monitors rolling revenue patterns for drops and spikes. |
| `revenue_per_employee_calculator` | Calculate revenue per employee and compare against industry benchmarks. |
...
| `revolving_credit_analyzer` | Analyze revolving credit costs including annual interest cost, utilization ratio, and cost per dollar borrowed. |
| `rewards_value_calculator` | Calculate the annual dollar value of credit card rewards based on monthly spending, rewards rate, and point value. |
...
| `risk_management_guide` | Returns educational content on types of financial risk, mitigation strategies, and hedging basics. |
...
| `risk_reward_ratio_calculator` | Calculate risk-reward ratio from entry price, stop-loss, and take-profit levels. Also computes breakeven win rate. |
...
| `sales_tax_calculator` | Calculate total sales tax from state and local rates. Returns tax amount, total cost, and combined rate. |
...
| `savings_account_comparator` | Compare earnings across multiple savings accounts for a given principal over 1, 3, and 5 year horizons, ranked by returns. |
...
| `savings_rate_calculator` | Calculates savings rate as a percentage of gross income and classifies the result. |
...
| `service_status` | Check the status of a systemd user service. Returns active state, sub-state, load state, description, and last activation timestamp. Example service names: 'snowdrop-mcp', 'openclaw-gateway'. |
...
| `shell_exec` | Run a shell command on the local system and return stdout, stderr, and exit code. Only commands whose first token(s) match an approved allowlist are permitted. Allowed prefixes: curl, wget, python3, python, git, systemctl --user, journalctl, ps, top, df, free, uptime, cat, ls, find. |
| `short_interest_crunch_indicator` | Combines short interest days-to-cover with catalyst calendar to time squeezes. |
| `side_hustle_profit_calculator` | Calculate profit, effective hourly rate, and estimated tax liability from a side hustle or gig work. |
| `signature_verifier` | Verifies HMAC-SHA256 signatures for incoming agent messages. |
| `sinking_fund_calculator` | Calculates the monthly contribution needed to save a target amount by a deadline. |
...
| `skill_quality_auditor` | AST-based security and quality audit for submitted skill code. Checks for dangerous imports, unsafe builtins, and TOOL_META compliance. Returns A2A-compliant feedback for revision requests. |
...
| `slack_post` | Post a message to the Snowdrop Slack channel. Used for daily engagement reports, milestone alerts, and status updates to Thunder. Requires SLACK_BOT_TOKEN and SLACK_CHANNEL_ID environment variables. |
...
| `social_security_benefit_estimator` | Estimate Social Security retirement benefits from average indexed monthly earnings (AIME). Calculates Primary Insurance Amount (PIA) and adjusts for early/late claiming. |
...
| `solar_panel_roi_calculator` | Calculate solar panel ROI including federal ITC, annual degradation, payback period, and 25-year total return. |
...
| `sortino_ratio_calculator` | Calculates the Sortino ratio, a variation of the Sharpe ratio that only penalizes downside volatility rather than total volatility. |
...
| `sp500_pe_ratio_tracker` | Get S&P 500 price-to-earnings ratio with historical comparison. Uses hardcoded recent data and historical averages. |
...
| `standard_vs_itemized_deduction_comparator` | Compare standard deduction vs itemized deductions (mortgage interest, SALT capped at $10k, charitable, medical over 7.5% AGI) and recommend the higher deduction for 2024. |
...
| `state_tax_comparator` | Compare effective state income tax rates for a given income across specified states. Hardcoded rates for the top 10 US states by population. |
...
| `sterling_ratio_calculator` | Calculates the Sterling ratio, comparing annualized return to average maximum drawdown plus a 10% buffer. |
...
| `stock_split_guide` | Returns educational content on stock splits: forward/reverse splits, reasons, and effects on value. |
| `stock_to_flow_calculator` | Applies the PlanB stock-to-flow regression to estimate model price and scarcity context. |
| `stock_valuation_guide` | Returns educational content on stock valuation methods: DCF, P/E, P/B, DDM with usage guidance and limitations. |
...
| `student_loan_payoff_calculator` | Calculate student loan payoff timeline, total interest paid, and show accelerated payment scenarios. |
...
| `subscription_cost_analyzer` | Analyzes subscription costs: total monthly and annual spending, sorted by cost. |
...
| `supply_demand_guide` | Returns educational content on supply and demand: law of supply/demand, equilibrium, elasticity, and shifts. |
...
| `swap_points_calculator` | Calculate FX swap points (forward points) from spot rate and interest rate differential. Swap points = forward - spot, expressed in pips. |
...
| `tax_bracket_calculator` | Calculate federal income tax owed using 2024 tax brackets. Shows marginal rate, effective rate, and tax owed per bracket. |
...
| `tax_deferred_accounts_guide` | Returns educational content on tax-advantaged accounts: 401(k), IRA, Roth, HSA, contribution limits, and RMDs. |
...
| `term_vs_whole_life_comparator` | Compare term vs whole life insurance. Calculates total cost of each and models investing the premium difference at a given return rate. |
...
| `time_value_of_money_guide` | Returns educational content on time value of money: PV, FV, annuities, perpetuities, and discount rates. |
| `timezone_scheduler` | Converts event timestamps into relevant time zones and flags off-hour meetings. |
| `tip_calculator` | Calculates tip amount, total bill, and per-person cost with optional group split. |
...
| `total_cost_of_credit_calculator` | Calculate total cost of credit: total paid, total interest, and effective rate for a fixed-rate loan. |
| `total_return_calculator` | Calculates total return as a percentage, including both capital appreciation and dividend income. |
| `tracking_error_calculator` | Calculates the annualized tracking error (standard deviation of active returns) between a portfolio and its benchmark. |
...
| `trade_balance_tracker` | Track the US trade balance (goods and services) from FRED (series BOPGSTB). Returns latest value. Requires FRED_API_KEY. |
...
| `treasury_yield_fetcher` | Fetch average interest rates for US Treasury securities from the Fiscal Data API. No API key required. |
| `trend_line_calculator` | Constructs trendlines using linear regression or peak/trough anchors to monitor price breaks. |
| `treynor_ratio_calculator` | Calculates the Treynor ratio, measuring excess return per unit of systematic risk (beta). |
...
| `trix_indicator_calculator` | Calculate the TRIX indicator, a momentum oscillator based on the rate of change of a triple-smoothed EMA. Filters out insignificant price movements. |
...
| `umbrella_policy_analyzer` | Analyze umbrella insurance needs: recommended coverage based on total assets minus existing liability coverage, with premium estimate. |
...
| `unemployment_rate_tracker` | Track the US unemployment rate (BLS series LNS14000000). Uses BLS API if BLS_API_KEY is set, otherwise returns static recent data. |
| `unit_economics_analyzer` | Analyze unit economics including gross margin, LTV:CAC ratio, and CAC payback period. |
...
| `utility_bill_analyzer` | Analyze a series of monthly utility bills to find average cost per kWh, seasonal patterns, highest/lowest months, and overall trend. |
| `vacancy_rate_calculator` | Calculate vacancy rate from total and vacant units. Estimates revenue loss based on vacancy. |
| `value_at_risk` | Computes multi-level VaR and CVaR via historical simulation. |
| `value_at_risk_calculator` | Calculates historical Value at Risk (VaR) at a given confidence level, representing the maximum expected loss over a period. |
...
| `vix_level_fetcher` | Fetch the CBOE Volatility Index (VIX) from FRED (series VIXCLS). Returns latest level with interpretation. Requires FRED_API_KEY. |
...
| `volatility_calculator` | Calculates historical volatility (standard deviation of returns), optionally annualized using sqrt(252) for daily data. |
...
| `williams_percent_r_calculator` | Calculate Williams %R, a momentum oscillator ranging from -100 to 0. Readings above -20 are overbought; below -80 are oversold. |
| `wire_transfer_fee_calculator` | Calculate total cost of a wire transfer including fees, and express the fee as a percentage of the transfer amount. |
| `withholding_calculator` | Calculate federal income tax withholding per pay period by annualizing gross pay, applying 2024 tax brackets, and dividing back to per-period amounts. |
...
| `working_capital_calculator` | Calculates net working capital and the current ratio to assess short-term liquidity. |
...
| `yield_curve_fetcher` | Fetch the US Treasury yield curve across multiple maturities (1M to 30Y) from the Treasury Fiscal Data API. Includes inversion detection. No API key required. |
| `yield_farming_apy_calculator` | Translates per-block reward rates into APR and APY estimates for yield farmers. |
| `yield_spread_calculator` | Calculate the yield spread between a long-term and short-term interest rate. Detects yield curve inversion and provides historical context. |
| `zero_based_budget_builder` | Builds a zero-based budget: every dollar of income is assigned to a category. Shows remaining unallocated funds. |
