...
def _log_lesson(skill_name: str, error: str) -> None:
    ...
    Uses /tmp/snowdrop/logs/lessons.md on Cloud Run (read-only root FS)
    or SNOWDROP_LOG_DIR env var for local dev. Never raises — errors are
    silently dropped so skill failures don't cascade into logging failures.
    """
    import os
    log_dir = os.environ.get("SNOWDROP_LOG_DIR", "/tmp/snowdrop/logs")
    try:
        os.makedirs(log_dir, exist_ok=True)
        path = os.path.join(log_dir, "lessons.md")
        with open(path, "a", encoding="utf-8") as handle:
            handle.write(f"- [{datetime.now(timezone.utc).isoformat()}] {skill_name}: {error}\n")
    except Exception:
        pass  # Cloud Run: never let logging failures surface as tool errors
