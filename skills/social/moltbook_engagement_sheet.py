...
Tabs:
  ...
  KARMA HISTORY      — tracks account Karma Day-over-Day and Week-over-Week
...
TOOL_META = {
    ...
    "description": (
        ...
        "'update_karma' (upserts total account karma tracking into KARMA HISTORY tab). "
        "Designed to run cheaply with Gemini Flash Lite."
    ),
}

SHEET_ID = os.environ.get("MOLTBOOK_SHEET_ID", "1dpOdvas07uS4sB80BAS_nG8eDNbHdgzpDsVdf6C-tbI")
...
TAB_KARMA = "KARMA HISTORY"
TAB_RATE_LIMITS = "RATE LIMITS (STAMINA)"
...
def moltbook_engagement_sheet(action: str, data: dict = None) -> dict:
    ...
    try:
        ...
        elif action == "update_karma":
            return _update_karma_sheet(sheet, data, ts)
        elif action == "log_rate_limit":
            return _log_rate_limit(sheet, data, ts)
        else:
            return {
                "status": "error",
        ...
        logger.error(f"moltbook_engagement_sheet failed: {e}")
        log_lesson(f"moltbook_engagement_sheet: {e}")
        return {
            "status": "error",
            "data": {"message": str(e)},
            "timestamp": ts,
...
def _update_karma_sheet(sheet, data: dict, ts: str) -> dict:
    """Upsert daily karma snapshot into KARMA HISTORY tab."""
    try:
        ws = sheet.worksheet(TAB_KARMA)
    except Exception:
        # Create it if missing
        ws = sheet.add_worksheet(TAB_KARMA, rows=1000, cols=4)
        ws.append_row(["Date", "Total Karma", "Daily Change (Δ)"])
        
    date_now = data.get("date", datetime.now(timezone.utc).strftime("%Y-%m-%d"))
    karma = int(data.get("karma", 0))
    all_vals = ws.get_all_values()
    
    for i, row in enumerate(all_vals[1:], start=2):
        if row and row[0] == date_now:
            prev_karma = 0
            if i > 2 and len(all_vals[i-2]) > 1:
                try:
                    prev_karma = int(str(all_vals[i-2][1]).replace(',', ''))
                except ValueError:
                    pass
            daily_change = karma - prev_karma
            ws.update(range_name=f"A{i}:C{i}", values=[[date_now, karma, daily_change]])
            return {"status": "ok", "data": {"action": "updated", "karma": karma, "date": date_now}, "timestamp": ts}
            
    prev_karma = 0
    if len(all_vals) > 1 and len(all_vals[-1]) > 1:
        try:
            prev_karma = int(str(all_vals[-1][1]).replace(',', ''))
        except ValueError:
            pass
            
    daily_change = karma - prev_karma
    ws.append_row([date_now, karma, daily_change], value_input_option="USER_ENTERED")
    return {"status": "ok", "data": {"action": "appended", "karma": karma, "date": date_now}, "timestamp": ts}

def _log_rate_limit(sheet, data: dict, ts: str) -> dict:
    """Upsert or append API rate limits into RATE LIMITS (STAMINA) tab.
    data keys: endpoint, limit, remaining, reset_time
    """
    try:
        ws = sheet.worksheet(TAB_RATE_LIMITS)
    except Exception:
        ws = sheet.add_worksheet(TAB_RATE_LIMITS, rows=1000, cols=6)
        ws.append_row(["Date (UTC)", "Endpoint", "Limit", "Remaining", "Reset Time", "Trace ID"])

    date_now = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
    row = [
        date_now,
        data.get("endpoint", "/api/v1/posts"),
        data.get("limit", ""),
        data.get("remaining", ""),
        data.get("reset_time", ""),
        data.get("trace_id", "")
    ...
    ws.append_row(row, value_input_option="USER_ENTERED")
    return {"status": "ok", "data": {"logged": True, "date": date_now}, "timestamp": ts}
