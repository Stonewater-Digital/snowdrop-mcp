...
# Paths — relative to the snowdrop-core repo root
_REPO_ROOT = Path(__file__).parent.parent.parent
_STATE_FILE = _REPO_ROOT / "logs" / "poller_state.json"
...
def performance_poller_control(action: str, limit: int = 50) -> dict:
    ...
    try:
        ...
        logger.error(f"performance_poller_control failed: {e}")
        log_lesson(f"performance_poller_control: {e}")
        return {
            "status": "error",
            "data": {"message": str(e)},
            "timestamp": ts,
