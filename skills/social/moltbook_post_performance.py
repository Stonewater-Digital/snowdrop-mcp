...
def moltbook_post_performance(post_ids: list, write_to_sheet: bool = False) -> dict:
    ...
    for post_id in post_ids:
        try:
            ...
            comments = post.get("comment_count", post.get("comments_count", 0)) or 0
            ...
            logger.error(f"moltbook_post_performance failed for {post_id}: {e}")
            log_lesson(f"moltbook_post_performance [{post_id}]: {e}")
    ...
    if write_to_sheet and results:
        try:
            ...
            logger.error(f"moltbook_post_performance sheet write failed: {e}")
            log_lesson(f"moltbook_post_performance (sheet write): {e}")
