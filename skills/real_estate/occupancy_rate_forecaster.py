...
from skills.utils import SkillTelemetryEmitter, get_iso_timestamp, log_lesson, logger
...
def occupancy_rate_forecaster(
    ...
    **kwargs: Any
    ...
    emitter = SkillTelemetryEmitter(
        "occupancy_rate_forecaster",
        {
            "historical_count": len(historical_rates or []),
            "market_adjusted": market_absorption_rate is not None or new_supply_pct is not None,
        },
    )
    try:
        ...
        log_lesson(f"occupancy_rate_forecaster: {e}")
        emitter.record("error", {"error": str(e)})
        return {
            "status": "error",
            "data": {"error": str(e)},
            "timestamp": get_iso_timestamp()
