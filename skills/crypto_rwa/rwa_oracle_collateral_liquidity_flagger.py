"""Executive Summary: Flags collateral where oracle liquidity inputs diverge from on-chain trades.

Focus: Liquidity coverage within the oracle reconciliation pillar.
"""
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_oracle_collateral_liquidity_flagger",
    "description": "Flags collateral where oracle liquidity inputs diverge from on-chain trades.",
    "tier": "free",
}


def rwa_oracle_collateral_liquidity_flagger(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_oracle_collateral_liquidity_flagger",
            description="Flags collateral where oracle liquidity inputs diverge from on-chain trades.",
            payload=payload,
            focus_tag="liquidity",
        ...
        _log_lesson("rwa_oracle_collateral_liquidity_flagger", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
