"""Executive Summary: Models share issuance math to catch dilution vectors for vault depositors.
...
TOOL_META: dict[str, Any] = {
    "name": "smart_contract_vault_share_dilution_checker",
    "description": "Models share issuance math to catch dilution vectors for vault depositors.",
    "tier": "free",
}


def smart_contract_vault_share_dilution_checker(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="smart_contract_vault_share_dilution_checker",
            description="Models share issuance math to catch dilution vectors for vault depositors.",
            payload=payload,
        ...
        _log_lesson("smart_contract_vault_share_dilution_checker", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
