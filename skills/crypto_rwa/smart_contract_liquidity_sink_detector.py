"""Executive Summary: Detects pools where withdrawals can be blocked via hooks or sticky fee logic.
...
TOOL_META: dict[str, Any] = {
    "name": "smart_contract_liquidity_sink_detector",
    "description": "Detects pools where withdrawals can be blocked via hooks or sticky fee logic.",
    "tier": "free",
}


def smart_contract_liquidity_sink_detector(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="smart_contract_liquidity_sink_detector",
            description="Detects pools where withdrawals can be blocked via hooks or sticky fee logic.",
            payload=payload,
            focus_tag="liquidity",
        ...
        _log_lesson("smart_contract_liquidity_sink_detector", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
