"""Executive Summary: Ensures no function can sweep native chain assets without quorum approval.

Focus: Custody coverage within the security heuristics pillar.
"""
...
TOOL_META: dict[str, Any] = {
    "name": "smart_contract_native_asset_sweep_detector",
    "description": "Ensures no function can sweep native chain assets without quorum approval.",
    "tier": "free",
}


def smart_contract_native_asset_sweep_detector(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    """Evaluate control health for Custody.
    ...
    try:
        data = analyze_payload(
            skill_name="smart_contract_native_asset_sweep_detector",
            description="Ensures no function can sweep native chain assets without quorum approval.",
            payload=payload,
            focus_tag="custody",
        ...
        _log_lesson("smart_contract_native_asset_sweep_detector", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
