"""Executive Summary: Diffs oracle values across chains to detect delayed relays or bridge drifts.

Focus: Bridge coverage within the oracle reconciliation pillar.
"""
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_oracle_cross_chain_feed_diff",
    "description": "Diffs oracle values across chains to detect delayed relays or bridge drifts.",
    "tier": "free",
}


def rwa_oracle_cross_chain_feed_diff(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_oracle_cross_chain_feed_diff",
            description="Diffs oracle values across chains to detect delayed relays or bridge drifts.",
            payload=payload,
            focus_tag="bridge",
        ...
        _log_lesson("rwa_oracle_cross_chain_feed_diff", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
