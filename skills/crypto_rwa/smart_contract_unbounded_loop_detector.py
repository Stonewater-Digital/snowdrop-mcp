"""Executive Summary: Scans bytecode for unbounded loops that can be griefed via block gas limits.
...
TOOL_META: dict[str, Any] = {
    "name": "smart_contract_unbounded_loop_detector",
    "description": "Scans bytecode for unbounded loops that can be griefed via block gas limits.",
    "tier": "free",
}


def smart_contract_unbounded_loop_detector(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="smart_contract_unbounded_loop_detector",
            description="Scans bytecode for unbounded loops that can be griefed via block gas limits.",
            payload=payload,
            focus_tag="gas",
        ...
        _log_lesson("smart_contract_unbounded_loop_detector", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
