"""Executive Summary: Ensures only approved CUSIPs appear in wrapped Treasury vaults by reconciling mint payloads with issuance calendars.

Focus: Treasury coverage within the asset verification pillar.
"""
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_treasury_cusip_whitelist_verifier",
...
def rwa_treasury_cusip_whitelist_verifier(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    """Evaluate control health for Treasury.
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_treasury_cusip_whitelist_verifier",
            description="Ensures only approved CUSIPs appear in wrapped Treasury vaults by reconciling mint payloads with issuance calendars.",
            payload=payload,
            focus_tag="treasury",
        ...
        _log_lesson("rwa_treasury_cusip_whitelist_verifier", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
