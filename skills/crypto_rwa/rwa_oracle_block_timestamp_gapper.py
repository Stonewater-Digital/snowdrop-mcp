"""Executive Summary: Measures block timestamp gaps affecting TWAP-driven oracle updates.

Focus: Time coverage within the oracle reconciliation pillar.
"""
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_oracle_block_timestamp_gapper",
    "description": "Measures block timestamp gaps affecting TWAP-driven oracle updates.",
    "tier": "free",
}


def rwa_oracle_block_timestamp_gapper(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    """Evaluate control health for Time.
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_oracle_block_timestamp_gapper",
            description="Measures block timestamp gaps affecting TWAP-driven oracle updates.",
            payload=payload,
            focus_tag="time",
        ...
        _log_lesson("rwa_oracle_block_timestamp_gapper", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
