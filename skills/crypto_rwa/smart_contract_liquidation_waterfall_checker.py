"""Executive Summary: Validates liquidation waterfalls respect seniority rules across tranches.

Focus: Risk coverage within the security heuristics pillar.
"""
...
TOOL_META: dict[str, Any] = {
    "name": "smart_contract_liquidation_waterfall_checker",
    "description": "Validates liquidation waterfalls respect seniority rules across tranches.",
    "tier": "free",
}


def smart_contract_liquidation_waterfall_checker(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    """Evaluate control health for Risk.
    ...
    try:
        data = analyze_payload(
            skill_name="smart_contract_liquidation_waterfall_checker",
            description="Validates liquidation waterfalls respect seniority rules across tranches.",
            payload=payload,
            focus_tag="risk",
        ...
        _log_lesson("smart_contract_liquidation_waterfall_checker", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
