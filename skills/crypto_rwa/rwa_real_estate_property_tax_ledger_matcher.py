"""Executive Summary: Matches property tax receipts versus blockchain cash flows to confirm taxes remain current.
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_real_estate_property_tax_ledger_matcher",
    "description": "Matches property tax receipts versus blockchain cash flows to confirm taxes remain current.",
    "tier": "free",
}


def rwa_real_estate_property_tax_ledger_matcher(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_real_estate_property_tax_ledger_matcher",
            description="Matches property tax receipts versus blockchain cash flows to confirm taxes remain current.",
            payload=payload,
        ...
        _log_lesson("rwa_real_estate_property_tax_ledger_matcher", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
