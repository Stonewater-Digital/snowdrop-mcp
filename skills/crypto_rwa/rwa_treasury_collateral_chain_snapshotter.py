"""Executive Summary: Builds snapshots of collateral pledges to avoid double counting the same bills.
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_treasury_collateral_chain_snapshotter",
    "description": "Builds snapshots of collateral pledges to avoid double counting the same bills.",
    "tier": "free",
}


def rwa_treasury_collateral_chain_snapshotter(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_treasury_collateral_chain_snapshotter",
            description="Builds snapshots of collateral pledges to avoid double counting the same bills.",
            payload=payload,
            focus_tag="treasury",
        ...
        _log_lesson("rwa_treasury_collateral_chain_snapshotter", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
