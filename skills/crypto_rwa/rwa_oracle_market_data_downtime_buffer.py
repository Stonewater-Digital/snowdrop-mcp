"""Executive Summary: Projects NAV impact when core venues go dark and oracles rely on stale buffers.

Focus: Resilience coverage within the oracle reconciliation pillar.
"""
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_oracle_market_data_downtime_buffer",
    "description": "Projects NAV impact when core venues go dark and oracles rely on stale buffers.",
    "tier": "free",
}


def rwa_oracle_market_data_downtime_buffer(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    """Evaluate control health for Resilience.
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_oracle_market_data_downtime_buffer",
            description="Projects NAV impact when core venues go dark and oracles rely on stale buffers.",
            payload=payload,
            focus_tag="resilience",
        ...
        _log_lesson("rwa_oracle_market_data_downtime_buffer", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
