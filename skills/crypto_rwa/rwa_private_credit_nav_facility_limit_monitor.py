"""Executive Summary: Checks NAV facility utilization versus limits before approving new draws.
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_private_credit_nav_facility_limit_monitor",
    "description": "Checks NAV facility utilization versus limits before approving new draws.",
    "tier": "free",
}


def rwa_private_credit_nav_facility_limit_monitor(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_private_credit_nav_facility_limit_monitor",
            description="Checks NAV facility utilization versus limits before approving new draws.",
            payload=payload,
        ...
        _log_lesson("rwa_private_credit_nav_facility_limit_monitor", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
