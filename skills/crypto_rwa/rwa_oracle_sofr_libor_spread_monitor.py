"""Executive Summary: Tracks SOFR-LIBOR spreads embedded in oracle discount factors for loans.

Focus: Rates coverage within the oracle reconciliation pillar.
"""
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_oracle_sofr_libor_spread_monitor",
    "description": "Tracks SOFR-LIBOR spreads embedded in oracle discount factors for loans.",
    "tier": "free",
}


def rwa_oracle_sofr_libor_spread_monitor(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_oracle_sofr_libor_spread_monitor",
            description="Tracks SOFR-LIBOR spreads embedded in oracle discount factors for loans.",
            payload=payload,
            focus_tag="rates",
        ...
        _log_lesson("rwa_oracle_sofr_libor_spread_monitor", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
