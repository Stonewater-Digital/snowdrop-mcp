"""Executive Summary: Enforces geo-fencing policies at the smart-contract level before transfers settle.
...
TOOL_META: dict[str, Any] = {
    "name": "token_standard_geofence_policy_checker",
    "description": "Enforces geo-fencing policies at the smart-contract level before transfers settle.",
    "tier": "free",
}


def token_standard_geofence_policy_checker(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="token_standard_geofence_policy_checker",
            description="Enforces geo-fencing policies at the smart-contract level before transfers settle.",
            payload=payload,
            focus_tag="policy",
        ...
        _log_lesson("token_standard_geofence_policy_checker", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
