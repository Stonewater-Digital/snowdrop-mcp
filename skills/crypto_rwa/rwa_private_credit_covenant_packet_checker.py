"""Executive Summary: Parses covenant reports to ensure breach notices trigger token gating logic.
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_private_credit_covenant_packet_checker",
    "description": "Parses covenant reports to ensure breach notices trigger token gating logic.",
    "tier": "free",
}


def rwa_private_credit_covenant_packet_checker(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_private_credit_covenant_packet_checker",
            description="Parses covenant reports to ensure breach notices trigger token gating logic.",
            payload=payload,
        ...
        _log_lesson("rwa_private_credit_covenant_packet_checker", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
