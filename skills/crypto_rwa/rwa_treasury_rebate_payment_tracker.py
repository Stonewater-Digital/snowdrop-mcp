"""Executive Summary: Tracks rebate and coupon-equivalent flows into token treasury accounts.
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_treasury_rebate_payment_tracker",
    "description": "Tracks rebate and coupon-equivalent flows into token treasury accounts.",
    "tier": "free",
}


def rwa_treasury_rebate_payment_tracker(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_treasury_rebate_payment_tracker",
            description="Tracks rebate and coupon-equivalent flows into token treasury accounts.",
            payload=payload,
            focus_tag="treasury",
        ...
        _log_lesson("rwa_treasury_rebate_payment_tracker", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
