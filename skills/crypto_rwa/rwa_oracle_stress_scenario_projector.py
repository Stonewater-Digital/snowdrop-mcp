"""Executive Summary: Applies stress shocks to oracle inputs to preview NAV drawdowns before they post.

Focus: Stress coverage within the oracle reconciliation pillar.
"""
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_oracle_stress_scenario_projector",
    "description": "Applies stress shocks to oracle inputs to preview NAV drawdowns before they post.",
    "tier": "free",
}


def rwa_oracle_stress_scenario_projector(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    """Evaluate control health for Stress.
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_oracle_stress_scenario_projector",
            description="Applies stress shocks to oracle inputs to preview NAV drawdowns before they post.",
            payload=payload,
            focus_tag="stress",
        ...
        _log_lesson("rwa_oracle_stress_scenario_projector", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
