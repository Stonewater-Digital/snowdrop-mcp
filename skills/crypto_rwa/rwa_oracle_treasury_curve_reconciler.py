"""Executive Summary: Compares oracle Treasury curves with FRED benchmarks to ensure discount factors match.

Focus: Fixed Income coverage within the oracle reconciliation pillar.
"""
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_oracle_treasury_curve_reconciler",
    "description": "Compares oracle Treasury curves with FRED benchmarks to ensure discount factors match.",
    "tier": "free",
}


def rwa_oracle_treasury_curve_reconciler(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    """Evaluate control health for Fixed Income.
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_oracle_treasury_curve_reconciler",
            description="Compares oracle Treasury curves with FRED benchmarks to ensure discount factors match.",
            payload=payload,
            focus_tag="fixed_income",
        ...
        _log_lesson("rwa_oracle_treasury_curve_reconciler", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
