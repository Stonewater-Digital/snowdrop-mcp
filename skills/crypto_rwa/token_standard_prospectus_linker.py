"""Executive Summary: Ensures wallets receive the latest prospectus hash before participating in offerings.

Focus: Disclosure coverage within the token compliance pillar.
"""
...
TOOL_META: dict[str, Any] = {
    "name": "token_standard_prospectus_linker",
    "description": "Ensures wallets receive the latest prospectus hash before participating in offerings.",
    "tier": "free",
}


def token_standard_prospectus_linker(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    """Evaluate control health for Disclosure.
    ...
    try:
        data = analyze_payload(
            skill_name="token_standard_prospectus_linker",
            description="Ensures wallets receive the latest prospectus hash before participating in offerings.",
            payload=payload,
            focus_tag="disclosure",
        ...
        _log_lesson("token_standard_prospectus_linker", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
