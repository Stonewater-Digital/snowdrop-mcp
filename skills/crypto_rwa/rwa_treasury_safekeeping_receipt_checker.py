"""Executive Summary: Validates safekeeping receipts and custodial chain-of-control for token wrappers.
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_treasury_safekeeping_receipt_checker",
    "description": "Validates safekeeping receipts and custodial chain-of-control for token wrappers.",
    "tier": "free",
}


def rwa_treasury_safekeeping_receipt_checker(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_treasury_safekeeping_receipt_checker",
            description="Validates safekeeping receipts and custodial chain-of-control for token wrappers.",
            payload=payload,
            focus_tag="treasury",
        ...
        _log_lesson("rwa_treasury_safekeeping_receipt_checker", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
