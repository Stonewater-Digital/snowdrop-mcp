"""Executive Summary: Measures latency when feeds fall back to secondary providers during outages.
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_oracle_fallback_latency_monitor",
    "description": "Measures latency when feeds fall back to secondary providers during outages.",
    "tier": "free",
}


def rwa_oracle_fallback_latency_monitor(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_oracle_fallback_latency_monitor",
            description="Measures latency when feeds fall back to secondary providers during outages.",
            payload=payload,
        ...
        _log_lesson("rwa_oracle_fallback_latency_monitor", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
