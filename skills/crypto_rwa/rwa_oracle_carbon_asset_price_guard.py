"""Executive Summary: Validates carbon offset price feeds against EU ETS and voluntary market data.

Focus: Carbon coverage within the oracle reconciliation pillar.
"""
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_oracle_carbon_asset_price_guard",
    "description": "Validates carbon offset price feeds against EU ETS and voluntary market data.",
    "tier": "free",
}


def rwa_oracle_carbon_asset_price_guard(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    """Evaluate control health for Carbon.
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_oracle_carbon_asset_price_guard",
            description="Validates carbon offset price feeds against EU ETS and voluntary market data.",
            payload=payload,
            focus_tag="carbon",
        ...
        _log_lesson("rwa_oracle_carbon_asset_price_guard", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
