"""Executive Summary: Aligns municipal bond marks from EMMA with on-chain oracle quotes and flags spreads beyond tolerance.
...
TOOL_META: dict[str, Any] = {
    "name": "rwa_oracle_muni_bond_price_sync",
    "description": "Aligns municipal bond marks from EMMA with on-chain oracle quotes and flags spreads beyond tolerance.",
    "tier": "free",
}


def rwa_oracle_muni_bond_price_sync(payload: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        data = analyze_payload(
            skill_name="rwa_oracle_muni_bond_price_sync",
            description="Aligns municipal bond marks from EMMA with on-chain oracle quotes and flags spreads beyond tolerance.",
            payload=payload,
        ...
        _log_lesson("rwa_oracle_muni_bond_price_sync", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
            "timestamp": timestamp,
