"""Firebase Realtime Database delete skill.

Deletes a node from Firebase Realtime Database using the REST API
...
logger = logging.getLogger("snowdrop.firebase_realtime_db_delete")

TOOL_META = {
    "name": "firebase_realtime_db_delete",
...
def firebase_realtime_db_delete(
    path: str,
    database_url: str | None = None,
) -> dict:
    """Delete a node from Firebase Realtime Database at the given path.

    Performs an authenticated HTTP DELETE against the Realtime Database REST
    endpoint. Deleting a non-existent path is considered a success (idempotent).

    Args:
        path: Database path to delete, e.g. "/users/123" or "/sessions/expired_token".
            Must start with "/". Deleting a path removes the node and all children.
    ...
    Returns:
        dict: Standard Snowdrop response envelope.
            On success: {"status": "ok", "data": {"path": str, "deleted": True}, "timestamp": str}
    ...
    Example:
        >>> result = firebase_realtime_db_delete(path="/sessions/stale_session_abc")
        >>> result["status"]
        'ok'
        >>> result["data"]["deleted"]
        True
    """
    logger.info("firebase_realtime_db_delete entered | path=%s", path)
    ...
    if not db_url:
        ...
        logger.error("firebase_realtime_db_delete error: %s", msg)
    ...
    try:
        ...
        with httpx.Client(timeout=15.0) as client:
            response = client.delete(url, params={"access_token": token})
            response.raise_for_status()

        logger.info("firebase_realtime_db_delete success | path=%s", path)
        return {
            "status": "ok",
        ...
        logger.error("firebase_realtime_db_delete error: %s", exc, exc_info=True)
