"""Simulates Frax IL exposure for dual-asset pools and recommends caps."""
...
# Frax is a hybrid stablecoin AMM (FRAX/USDC, frxETH/ETH pools); its AMO mechanism provides a
# moderate volatility buffer, warranting a threshold between pure stablecoins (Curve) and volatile AMMs.
_IL_VOLATILITY_THRESHOLD: float = 0.270

TOOL_META: dict[str, Any] = {
    "name": "frax_impermanent_loss_guardrail",
    "description": "Simulates Frax IL exposure for dual-asset pools and recommends caps.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def frax_impermanent_loss_guardrail(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    """Simulate Frax yield scenarios."""
    ...
    try:
        ...
        _log_lesson("frax_impermanent_loss_guardrail", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
