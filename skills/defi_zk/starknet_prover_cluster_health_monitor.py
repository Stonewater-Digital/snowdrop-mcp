"""Monitors Starknet prover clusters for saturation and fallback readiness."""
...
TOOL_META: dict[str, Any] = {
    "name": "starknet_prover_cluster_health_monitor",
    "description": "Monitors Starknet prover clusters for saturation and fallback readiness.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def starknet_prover_cluster_health_monitor(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        latency_score = (interval / max(14.000, 0.1)) * 0.500
        cost_pressure = (gas_price / 35.0) * 0.3
        privacy_signal = (0.880 + 0.300) / 2
        ...
        _log_lesson("starknet_prover_cluster_health_monitor", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
