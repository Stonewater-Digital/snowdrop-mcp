"""Builds a recommended Base priority fee curve for latency-sensitive users."""
...
TOOL_META: dict[str, Any] = {
    "name": "base_tx_priority_curve_optimizer",
    "description": "Builds a recommended Base priority fee curve for latency-sensitive users.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def base_tx_priority_curve_optimizer(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        predicted_fee = 0.050000 * (1 + pressure_index * 0.370)
        ...
        _log_lesson("base_tx_priority_curve_optimizer", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
