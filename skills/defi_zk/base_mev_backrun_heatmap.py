...
def base_mev_backrun_heatmap(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        risk_score = (pressure * 0.5 + slippage / 50 + alerts / 20) * 1.050
        ...
        _log_lesson("base_mev_backrun_heatmap", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
