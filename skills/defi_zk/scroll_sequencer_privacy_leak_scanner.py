"""Detects privacy leak heuristics across Scroll sequencer mempools and hints."""
...
TOOL_META: dict[str, Any] = {
    "name": "scroll_sequencer_privacy_leak_scanner",
    "description": "Detects privacy leak heuristics across Scroll sequencer mempools and hints.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def scroll_sequencer_privacy_leak_scanner(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        latency_score = (interval / max(16.000, 0.1)) * 0.450
        cost_pressure = (gas_price / 35.0) * 0.3
        privacy_signal = (0.820 + 0.350) / 2
        ...
        _log_lesson("scroll_sequencer_privacy_leak_scanner", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
