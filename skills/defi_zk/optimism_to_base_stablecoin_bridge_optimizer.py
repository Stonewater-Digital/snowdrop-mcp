"""Optimizes stablecoin bridge routing from Optimism to Base with fee and latency heuristics."""
...
TOOL_META: dict[str, Any] = {
    "name": "optimism_to_base_stablecoin_bridge_optimizer",
    "description": "Optimizes stablecoin bridge routing from Optimism to Base with fee and latency heuristics.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def optimism_to_base_stablecoin_bridge_optimizer(metrics: Mapping[str, float | int | str]) -> dict[str, Any]:
    """Route stablecoin transfers from Optimism to Base."""
    ...
    try:
        ...
        _log_lesson("optimism_to_base_stablecoin_bridge_optimizer", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
