"""Optimizes ETH bridge routing from Optimism to Base with fee and latency heuristics."""
...
TOOL_META: dict[str, Any] = {
    "name": "optimism_to_base_eth_bridge_optimizer",
    "description": "Optimizes ETH bridge routing from Optimism to Base with fee and latency heuristics.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def optimism_to_base_eth_bridge_optimizer(metrics: Mapping[str, float | int | str]) -> dict[str, Any]:
    """Route ETH transfers from Optimism to Base."""
    ...
    try:
        ...
        _log_lesson("optimism_to_base_eth_bridge_optimizer", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
