"""Optimizes governance token bridge routing from Base to Optimism with fee and latency heuristics."""
...
TOOL_META: dict[str, Any] = {
    "name": "base_to_optimism_govtoken_bridge_optimizer",
    "description": "Optimizes governance token bridge routing from Base to Optimism with fee and latency heuristics.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def base_to_optimism_govtoken_bridge_optimizer(metrics: Mapping[str, float | int | str]) -> dict[str, Any]:
    """Route governance token transfers from Base to Optimism."""
    ...
    try:
        ...
        _log_lesson("base_to_optimism_govtoken_bridge_optimizer", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
