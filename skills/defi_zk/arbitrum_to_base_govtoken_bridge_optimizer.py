"""Optimizes governance token bridge routing from Arbitrum to Base with fee and latency heuristics."""
...
TOOL_META: dict[str, Any] = {
    "name": "arbitrum_to_base_govtoken_bridge_optimizer",
    "description": "Optimizes governance token bridge routing from Arbitrum to Base with fee and latency heuristics.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def arbitrum_to_base_govtoken_bridge_optimizer(metrics: Mapping[str, float | int | str]) -> dict[str, Any]:
    """Route governance token transfers from Arbitrum to Base."""
    ...
    try:
        ...
        _log_lesson("arbitrum_to_base_govtoken_bridge_optimizer", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
