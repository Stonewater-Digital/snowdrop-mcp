...
TOOL_META: dict[str, Any] = {
    "name": "zksync_sequencer_privacy_leak_scanner",
...
def zksync_sequencer_privacy_leak_scanner(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        latency_score = (interval / max(12.000, 0.1)) * 0.450
        cost_pressure = (gas_price / 35.0) * 0.3
        privacy_signal = (0.900 + 0.350) / 2
        ...
        _log_lesson("zksync_sequencer_privacy_leak_scanner", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
