"""Optimizes ETH bridge routing from Base to Optimism with fee and latency heuristics."""
...
TOOL_META: dict[str, Any] = {
    "name": "base_to_optimism_eth_bridge_optimizer",
    "description": "Optimizes ETH bridge routing from Base to Optimism with fee and latency heuristics.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def base_to_optimism_eth_bridge_optimizer(metrics: Mapping[str, float | int | str]) -> dict[str, Any]:
    """Route ETH transfers from Base to Optimism."""
    ...
    try:
        ...
        _log_lesson("base_to_optimism_eth_bridge_optimizer", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
