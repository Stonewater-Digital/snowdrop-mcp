"""Monitors zkSync prover clusters for saturation and fallback readiness."""
...
TOOL_META: dict[str, Any] = {
    "name": "zksync_prover_cluster_health_monitor",
    "description": "Monitors zkSync prover clusters for saturation and fallback readiness.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def zksync_prover_cluster_health_monitor(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        latency_score = (interval / max(12.000, 0.1)) * 0.500
        cost_pressure = (gas_price / 35.0) * 0.3
        privacy_signal = (0.900 + 0.300) / 2
        ...
        _log_lesson("zksync_prover_cluster_health_monitor", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
