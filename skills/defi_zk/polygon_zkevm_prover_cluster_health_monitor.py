...
TOOL_META: dict[str, Any] = {
    "name": "polygon_zkevm_prover_cluster_health_monitor",
...
def polygon_zkevm_prover_cluster_health_monitor(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        latency_score = (interval / max(18.000, 0.1)) * 0.500
        cost_pressure = (gas_price / 35.0) * 0.3
        privacy_signal = (0.800 + 0.300) / 2
        ...
        _log_lesson("polygon_zkevm_prover_cluster_health_monitor", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
