"""Simulates StakeWise IL exposure for dual-asset pools and recommends caps."""
...
# IL volatility threshold — the volatility level above which IL risk becomes material for this protocol.
# StakeWise uses ossETH, a vault-based staking receipt tightly pegged to ETH. Its pairs are even
# more correlated than EtherFi, warranting the most conservative threshold of all IL guardrails.
_IL_VOLATILITY_THRESHOLD: float = 0.180

TOOL_META: dict[str, Any] = {
    "name": "stakewise_impermanent_loss_guardrail",
    "description": "Simulates StakeWise IL exposure for dual-asset pools and recommends caps.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def stakewise_impermanent_loss_guardrail(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    """Simulate StakeWise yield scenarios."""
    ...
    try:
        ...
        apr_adjusted = apr * (1 - volatility * 0.550)
        projected_yield = principal * (apr_adjusted / 100) * (lockup / 365)
        hedge_requirement = principal * volatility * 0.350
        il_floor = max(0.0, volatility - _IL_VOLATILITY_THRESHOLD) * 100
        ...
        _log_lesson("stakewise_impermanent_loss_guardrail", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
