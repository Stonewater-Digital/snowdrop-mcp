...
TOOL_META: dict[str, Any] = {
    "name": "zksync_state_diff_audit_simulator",
...
def zksync_state_diff_audit_simulator(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        latency_score = (interval / max(12.000, 0.1)) * 0.400
        cost_pressure = (gas_price / 35.0) * 0.3
        privacy_signal = (0.900 + 0.250) / 2
        ...
        _log_lesson("zksync_state_diff_audit_simulator", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
