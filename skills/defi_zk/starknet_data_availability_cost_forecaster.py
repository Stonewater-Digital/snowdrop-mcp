...
# Protocol-specific tuning constants — derived from Starknet mainnet observability data.
# STARK proofs (Cairo) have the fastest typical submission cadence and the highest
# native privacy coefficient of the six supported rollups.
_BASELINE_PROOF_INTERVAL_MIN: float = 14.0   # Typical proof submission cadence on Starknet mainnet
_PROTOCOL_PRIVACY_BASE: float = 0.880         # STARK proof system base privacy score (highest among ZK rollups)
_PRIVACY_OVERHEAD: float = 0.180              # Starknet volition-mode overhead factor

TOOL_META: dict[str, Any] = {
    "name": "starknet_data_availability_cost_forecaster",
...
def starknet_data_availability_cost_forecaster(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    """Model rollup proof health for Starknet."""
    ...
    try:
        ...
        _log_lesson("starknet_data_availability_cost_forecaster", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
