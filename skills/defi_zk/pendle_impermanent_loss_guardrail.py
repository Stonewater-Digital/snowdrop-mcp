...
# Pendle is a yield tokenization protocol; PT/YT time-decay naturally dampens short-term price noise,
# warranting the highest threshold of all IL guardrails — LPs tolerate more vol before IL is material.
_IL_VOLATILITY_THRESHOLD: float = 0.320

TOOL_META: dict[str, Any] = {
    "name": "pendle_impermanent_loss_guardrail",
...
def pendle_impermanent_loss_guardrail(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    """Simulate Pendle yield scenarios."""
    ...
    try:
        ...
        _log_lesson("pendle_impermanent_loss_guardrail", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
