...
# Curve is stablecoin-focused; tight peg dynamics mean even small volatility deviations cause
# disproportionate IL, requiring a conservative (low) threshold.
_IL_VOLATILITY_THRESHOLD: float = 0.220

TOOL_META: dict[str, Any] = {
    "name": "curve_impermanent_loss_guardrail",
...
def curve_impermanent_loss_guardrail(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        _log_lesson("curve_impermanent_loss_guardrail", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
