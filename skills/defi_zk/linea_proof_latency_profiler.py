...
TOOL_META: dict[str, Any] = {
    "name": "linea_proof_latency_profiler",
...
def linea_proof_latency_profiler(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        latency_score = (interval / max(15.000, 0.1)) * 0.600
        cost_pressure = (gas_price / 35.0) * 0.3
        privacy_signal = (0.840 + 0.200) / 2
        ...
        _log_lesson("linea_proof_latency_profiler", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
