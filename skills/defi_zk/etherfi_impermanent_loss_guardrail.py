"""Simulates EtherFi IL exposure for dual-asset pools and recommends caps."""
...
# EtherFi is an LST (liquid staking) protocol; pairs are ETH-correlated assets (eETH/ETH, weETH/ETH)
# which are tightly coupled, warranting the tightest threshold among non-stablecoin protocols.
_IL_VOLATILITY_THRESHOLD: float = 0.200

TOOL_META: dict[str, Any] = {
    "name": "etherfi_impermanent_loss_guardrail",
    "description": "Simulates EtherFi IL exposure for dual-asset pools and recommends caps.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def etherfi_impermanent_loss_guardrail(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        _log_lesson("etherfi_impermanent_loss_guardrail", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
