"""Detects Base latency breaker events before users feel sequencer stalls."""
...
TOOL_META: dict[str, Any] = {
    "name": "base_latency_breaker_detector",
    "description": "Detects Base latency breaker events before users feel sequencer stalls.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def base_latency_breaker_detector(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        predicted_fee = 0.050000 * (1 + pressure_index * 0.330)
        ...
        _log_lesson("base_latency_breaker_detector", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
