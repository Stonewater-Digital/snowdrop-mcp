...
TOOL_META: dict[str, Any] = {
    "name": "taiko_proof_latency_profiler",
...
def taiko_proof_latency_profiler(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        latency_score = (interval / max(17.000, 0.1)) * 0.600
        cost_pressure = (gas_price / 35.0) * 0.3
        privacy_signal = (0.860 + 0.200) / 2
        ...
        _log_lesson("taiko_proof_latency_profiler", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
