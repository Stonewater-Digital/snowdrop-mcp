"""Detects Optimism bundle collisions that hint at searcher wars."""
...
TOOL_META: dict[str, Any] = {
    "name": "optimism_bundle_collision_detector",
    "description": "Detects Optimism bundle collisions that hint at searcher wars.",
    "tier": "free",
    "inputSchema": {
    "type": "object",
    "properties": {
...
def optimism_bundle_collision_detector(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    """Estimate MEV pressure for Optimism."""
    ...
    try:
        ...
        risk_score = (pressure * 0.5 + slippage / 50 + alerts / 20) * 1.000
        ...
        _log_lesson("optimism_bundle_collision_detector", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
