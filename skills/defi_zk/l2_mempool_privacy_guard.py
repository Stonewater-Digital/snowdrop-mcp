...
def l2_mempool_privacy_guard(metrics: Mapping[str, float | int]) -> dict[str, Any]:
    ...
    try:
        ...
        risk_score = (pressure * 0.5 + slippage / 50 + alerts / 20) * 0.980
        ...
        _log_lesson("l2_mempool_privacy_guard", str(exc))
        return {
            "status": "error",
            "data": {"error": str(exc)},
